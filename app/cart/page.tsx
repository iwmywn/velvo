...
export default function CartPage() {
  return (
    <main className="relative z-10 px-8 pt-4 md:px-20">
      ...
      <div className="flex flex-col overflow-x-scroll text-sm min-[400px]:overflow-visible">
        <div className="mb-4 grid grid-cols-6 rounded-lg border sm:grid-cols-7">
          {heads.map((head, index) => (
            <div
              className={`flex items-center justify-center text-nowrap py-2 font-medium ${index === 0 && "col-span-2 sm:col-span-3"}`}
              key={head}
            >
              {head}
            </div>
        ...
        {mockProduct.map(({ href, name, price, quantity }, index) => (
          <div
            className="mb-2 grid grid-cols-6 rounded-lg border py-2 sm:grid-cols-7 md:py-0"
            key={index}
          >
            <div className="relative col-span-2 flex flex-col items-center justify-center gap-x-3 gap-y-1 p-1 text-center sm:col-span-3 md:flex-row md:justify-normal md:text-left">
